...
export function ProductCard({product}:{product:Product}){
  ...
  return <article className="group flex h-full min-w-0 flex-col overflow-hidden rounded-2xl bg-white transition duration-300 hover:-translate-y-1 hover:shadow-lift">
    <Link to={`/products/${product.slug}`} className="relative block overflow-hidden bg-rose-50">
      <img src={product.imagePath} alt={`${product.name} from ${websiteSettings.shopName}`} width="600" height="338" loading="lazy" className="aspect-video w-full object-cover transition duration-500 group-hover:scale-105"/>
    ...
    <div className="flex flex-1 flex-col p-3">
      <div className="flex items-center gap-2 text-[11px] font-bold text-slate-500"><span className="inline-flex items-center gap-1 text-leaf"><Star size={12} fill="currentColor"/> {rating}</span><span>·</span><span className="inline-flex items-center gap-1"><CalendarDays size={12}/> Pre-order</span></div>
      ...
      <div className="mt-auto flex items-end justify-between gap-2 pt-2">
        <div><span className="block text-base font-extrabold text-forest sm:text-lg">{formatCurrency(product.discountPrice)}</span><span className="text-[10px] font-semibold text-slate-400">Current listed rate</span>{product.originalPrice>product.discountPrice&&<del className="block text-[11px] text-slate-400">{formatCurrency(product.originalPrice)}</del>}</div>
