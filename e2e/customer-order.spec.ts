...
test('customer can order Mallige and Jaaji through checkout', async ({ page }) => {
  await page.addInitScript(() => { window.open = (url) => { sessionStorage.setItem('lastOpenedUrl', String(url)); return null; }; });
  await page.goto('./');
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Udupi Mallige');
  ...
  const atteCard = page.locator('article').filter({ hasText: /Mallige.*1 Atte/ });
  await atteCard.getByRole('button', { name: 'ADD' }).click();
  await atteCard.getByRole('button', { name: /Add one more/ }).click({ clickCount: 3 });
  await page.getByRole('link', { name: /Shopping cart with/ }).click();
...
test('products page shows all available flower formats', async ({ page }) => {
