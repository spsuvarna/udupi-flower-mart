...
import { ArrowLeft, CalendarDays, Check, Clock3, MessageCircle, ShoppingCart, Star } from 'lucide-react';
    ...
    <div className="container-page py-4 sm:py-6">
      ...
      <div className="mt-4 grid gap-7 lg:grid-cols-2">
        ...
        <div className="lg:py-3">
          <div className="flex flex-wrap gap-2"><span className={`badge ${product.available?'bg-green-100 text-green-800':'bg-slate-200 text-slate-700'}`}>{product.available?'Fresh & available':'Unavailable'}</span><span className="badge items-center bg-orange-100 text-orange-800"><CalendarDays size={14} className="mr-1"/>Order one day early</span></div>
          ...
          <p className="mt-2 flex items-center gap-2 text-sm font-bold text-saffron"><Clock3 size={17}/>Current listed price · Final rate follows your delivery day</p>
          ...
          <ul className="mt-7 space-y-2 border-t border-slate-200 pt-6 text-sm text-slate-600"><li className="flex gap-2"><Check className="text-leaf" size={20}/>Freshness and final availability confirmed personally</li><li className="flex gap-2"><Check className="text-leaf" size={20}/>Choose tomorrow or a later delivery date at checkout</li></ul>
        </div>
      </div>
